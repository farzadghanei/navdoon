...
class TCPClient(LoggerMixIn):
    ...
    def reconnect(self):
        ...
        if self._connection_tries >= self.max_retry:
            raise IOError("Reached maximum connection tries of '{}' to {}:{}".format(max_retry, self.host, self.port))
        self.disconnect()
        self.connect()
    ...
    def _send_with_lock(self, data_bytes):
        ...
        with self._sending_lock:
            while True:
                self._log_debug("sending {} bytes to {}:{} ...".format(data_size, self.host, self.port))
                sock = self._connection()
                try:
                    sock.sendall(data)
                    self._log_debug("sent {} bytes to {}:{}".format(data_size, self.host, self.port))
                    break
                except socket.error:
                    self._log_error("failed to send data to {}:{}. {}".format(self.host, self.port, err))
                    self.reconnect()
        ...
        with self._connection_lock:
            if not self._sock:
                ...
                while True:
                    ...
                    try:
                        self._log_debug("connecting to {}:{} try {}/{}".format(self.host, self.port, self._connection_tries, max_retry))
                        sock = socket.socket(socket.AF_INET, socket.SOCK_STRAEM)
                        sock.connect((self.host, self.port))
                        self._log_debug("connected to {}:{}".format(self.host, self.port))
                        self._sock = sock
                        break
                    except socket.error as err:
                        self._log_error("failed to connect to {}:{}. {}".format(self.host, self.port, err))
                        sock.close()
