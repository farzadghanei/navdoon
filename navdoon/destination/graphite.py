...
class Graphite(TCPClient):
    ...
    def _send_lines(self, lines):
        ...
        self._log_debug("flushing %d metrics to graphite on {}:{} ...".format(num_lines, self.host, self.port))
        self._send_with_lock(data)
        self._log("flushed %d metrics to graphite on {}:{}".format(num_lines, self.host, self.port))
